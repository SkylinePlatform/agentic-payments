...
//
// And it is not only a test double. It is what the demo runs on any machine
// with no key: deploy/demo.json gives agent-watch -interpreter auto, which
// builds this when GEMINI_API_KEY is unset, so no key and no network are needed
// and beat 2 has to happen for beats 3 to 10 to exist at all. A type declared in
// a _test.go file is reachable only from its own package's test binary, so
// cmd/agent could not name one — and cmd/agent does name one, through Demo(),
// whenever -interpreter resolves to scripted. internal/agent's own tests
