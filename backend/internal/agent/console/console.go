...
func (s *Service) list(w http.ResponseWriter, _ *http.Request) {
	...
	// grow a cursor or a count without every reader changing shape. Nothing has
	// grown there yet: `boot` did, and issue #320 took it away again when the
	// reader it was for stopped being able to exist.
	roles.OK(w, http.StatusOK, listed{Watches: out})
}
