...
// spine is drawn from, because DelegateCheckout and DelegatePayment handed
// back a signed chain and nothing else. CheckoutDigestOf and PaymentDigestOf
// are the accessor #156 argues for instead of a second, independently
// recomputing helper: they read checkout_hash / transaction_id back off the
// chain this package already signed, so there is exactly one computation of
// the digest — the one inside checkoutClaims/paymentClaims via bindClosed —
// and these decode its result rather than reaching it a second way.
//
// Both are proved against the same ground truth: the digest a real verifier
// reaches via AuthoriseCheckoutChain / AuthorisePaymentChain, which resolves
// the delegate payload through the full, verified processor rather than this
// package's own unverified read of the first disclosure. If the two ever
// disagreed, the accessor would be naming a different purchase from the one
// the chain actually authorises — the exact failure #156 exists to keep off
// the screen.
func TestCheckoutDigestOfReadsWhatDelegateCheckoutWrote(t *testing.T) {
	...
	assert.Equal(t, verified.Closed.CheckoutHash, got,
		"the accessor has to return the same digest a verifier independently confirms by resolving the whole delegate payload, not a value read some other way")
}
